...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_box" >
  ...
  <tr>
	...
	<br />Devam Etmek İstiyor musunuz?<br /><br /><a href="poll_comments.php">[ Hayır ]</a> | <a href="poll_comments.php?do=delete&comment_id=<?=$comment_id?>&del_ok=yes">[ Evet ]</a>
	<?php
	}elseif($del == "yes"){
	...
	<a href="poll_comments.php">Sayfaya Dönmek İçin Tıklayınız.</a>
	<?php
	}
	?>
	</td>
...
?>
	<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_box" >
    	  ...
    	  <td class="row5px listEven" align="center">
			...
			<a href="poll_comments.php">Lütfen Bekleyiniz... Sayfaya Yönlendiriliyorsunuz.</a>
			<meta http-equiv="Refresh" content="2; poll_comments.php" />
			<?php
			}
			?>
		  </td>
...
<?php
	...
	$query = $db->write_query("select c.comment_id, c.email, c.title, c.comment, c.name, c.ip, c.date, c.active, a.poll_title from comments as c, polls as a where c.id = a.poll_id and c.type = 'poll' and
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_box" >
  ...
  <tr>
    <td class="row5px listEven"><b>Anket   :</b></td>
    <td valign="middle" class="row5px listOdd">&nbsp;<?=stripslashes($row[poll_title])?></td>
  </tr>
  <tr>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_box" >
  ...
  $query = $db->write_query("select c.comment_id, c.name, c.ip, c.date, c.active, a.poll_title from comments as c,
  polls as a where c.id = a.poll_id and c.type = 'poll' order by c.active desc, c.date desc limit $start,$limit");
  ...
  <tr class="<?=$class?>">
	...
	<a class="blacklink" href="poll_comments.php?do=edit&comment_id=<?=$row[comment_id]?>">Düzenle</a> &nbsp;|&nbsp;
	<img src="images/delete.gif" width="17" height="17" align="absmiddle"> 
	<a class="blacklink" href="poll_comments.php?do=delete&comment_id=<?=$row[comment_id]?>">Sil</a> </td>
  </tr>
  <?php
  $i++;
  }
...
<div id="page_nav"><?php
echo generate_pagination("poll_comments.php?do", $comments_count, $limit, $start);?></div>
<?php
}
include "footer.php";
?>
