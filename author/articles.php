...
$article_id = ( isset($HTTP_GET_VARS['article_id']) ) ? intval($HTTP_GET_VARS['article_id']) : intval($HTTP_POST_VARS['article_id']);
...
$query = $db->read_query("select title from articles where article_id = $article_id AND author_id = ".$session_uid."");
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_box" >
  ...
  <tr>
	...
	$db->write_query("delete from articles where article_id = ".$article_id." AND author_id = ".$session_uid."");
...
if($do == "edit"){
	$query = $db->read_query("
							...
							from articles where article_id = $article_id AND author_id = ".$session_uid."
							");
	$row = $db->sql_fetcharray($query);
	}
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_box" >
  ...
  <tr>
    ...
    <td class="row5px listOdd"><select name="author_id" id="author_id" style="width:150px;">
      <?php
		$a_query = $db->read_query("Select author_id, name from authors WHERE author_id = ".$session_uid."");
...
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_box" >
  ...
  $i=0;
  $query = $db->write_query("select count(articles.article_id) as total from articles as articles, authors as authors where articles.author_id = authors.author_id AND articles.author_id = ".$session_uid." $add_query");
  ...
  $sql = "select 
		...
		AND authors.author_id = ".$session_uid."
...
<div id="page_nav"><?php
echo generate_pagination("articles.php?title=".stripslashes($title)."&order_type=$order_type&order=$order&start_time=$start_time&end_time=$end_time", $articles_count, $limit, $start);?></div>
<?php
}
